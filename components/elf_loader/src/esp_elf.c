...
static int esp_elf_load_segment(esp_elf_t *elf, const uint8_t *pbuf)
{
    ...
    if (size == 0) {
	ESP_LOGE(TAG, "esp_elf_load_segment size == 0");
        return -EINVAL;
    }
    ...
    if (!elf->psegment) {
	ESP_LOGE(TAG, "esp_elf_load_segment !elf->psegment");
        return -ENOMEM;
    }
...
int esp_elf_relocate(esp_elf_t *elf, const uint8_t *pbuf)
{
    ...
    if (!elf || !pbuf) {
	ESP_LOGW(TAG, "esp_elf_relocate !elf || !pbuf");
        return -EINVAL;
    }
...
#if CONFIG_ELF_LOADER_BUS_ADDRESS_MIRROR
        ESP_LOGE(TAG, "Error loading elf file (esp_elf_load_section failed), ret=%d", ret);
#else
        ESP_LOGE(TAG, "Error loading elf file (esp_elf_load_segment failed), ret=%d", ret);
#endif
        return ret;
    }
