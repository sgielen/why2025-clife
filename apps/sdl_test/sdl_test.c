...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]) {
    ...
    *appstate = as;
    ...
    // Create window first
    as->window = SDL_CreateWindow("examples/demo/snake", SDL_WINDOW_WIDTH, SDL_WINDOW_HEIGHT, 0);
    if (!as->window) {
    ...
    if (props) {
        ...
        if (formats) {
            ...
            for (int j = 0; formats[j] != SDL_PIXELFORMAT_UNKNOWN; j++) {
                SDL_Log("  Format %d: %s", j, SDL_GetPixelFormatName(formats[j]));
            }
        }
    }
...
SDL_AppResult SDL_AppEvent(void *appstate, SDL_Event *event) {
    ...
    switch (event->type) {
        ...
        case SDL_EVENT_JOYSTICK_REMOVED:
            ...
            break;
        case SDL_EVENT_JOYSTICK_HAT_MOTION: return handle_hat_event_(ctx, event->jhat.value);
