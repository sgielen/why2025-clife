...
static const keyboard_scancode_t keymap[80] = {
...
keyboard_event_ll_t scancode_to_event(uint8_t scancode) {
    ...
    keyboard_event_ll_t event;
    event.down       = scancode >> 7; // high bit set means pressed
    event.timestamp  = (int64_t)tv_now.tv_sec * 1000000L + (int64_t)tv_now.tv_usec;
    event.scancode   = s;
...
static ssize_t tca8418_read(void *dev, int fd, void *buf, size_t count) {
    ...
    while (written <= count) {
        if (tca8418_get_event_count(device->keyboard)) {
            ESP_LOGW(TAG, "Got keyboard event");
            char c = tca8418_get_key(device->keyboard);
            keyboard_event_ll_t event = scancode_to_event(c);
            if (written + sizeof(keyboard_event_ll_t) > count) {
                break;
            }
            memcpy(buf, &event, sizeof(keyboard_event_ll_t));
            written += sizeof(keyboard_event_ll_t);
        } else {
            break;
        }
    }
